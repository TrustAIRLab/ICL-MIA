...
def main(models, datasets, num_seeds, positions, all_shots):
    ...
    for param_index, params in enumerate(all_params):
        ...
        required_for_nonmem = gap_baseline(
            params,
            demo_sentences,
            demo_labels,
            nonmember_sentences,
...
def gap_baseline(params, train_sentences, train_labels, test_sentence, test_label):
    input_to_model = construct_prompt(
        params, train_sentences, train_labels, test_sentence
    )
    return_idx = response_from_model(params, input_to_model, llm_model, llm_tokenizer)
    if return_idx == test_label:
        return 1
    else:
        return 0
